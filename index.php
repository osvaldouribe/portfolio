<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
	<title>Osvaldo Uribe Escobar – Product Designer</title>
    <!-- Bootstrap CSS -->
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0-beta.2/css/bootstrap.min.css" integrity="sha384-PsH8R72JQ3SOdhVi3uxftmaW6Vc51MKb0q5P2rRUpPvrszuE4W1povHYgTpBfshb" crossorigin="anonymous">
    <!-- Custom Style -->
	<link rel="stylesheet" href="style.css?=v6">
  </head>
  <body>
	<header class="container">
		<div class="row">
			<div id="brand" class="col-md-6">
				<h1><a href="index.php">Osvaldo Uribe</a></h1>
			</div>
			<div id="navigation" class="col-md-6">
				<ul>
					<li><a href="services.php">Services</a>
					<!-- <li><a href="about.php">About</a> -->
				</ul>
			</div>
		</div>
	</header>
	
	<section class="container">
		<div class="row">
			<div class="col-md-10">
				<h2>Experienced product designer with over 15 years of versatile expertise across diverse markets.</h2>
			</div>
		</div>
		<div class="row">
			<div class="col-md-4">	
				<img src="img/paint-1.png" class="rounded img-fluid d-none d-lg-block">
			</div>
			<div class="col-md-6 offset-md-2">
				<img src="img/portrait-osvaldo.jpg" class="rounded img-fluid">
				<figcaption>Photo by <a href="https://www.instagram.com/johnmartinezphotos/">John Martinez</a></figcaption>
			</div>
		</div>
		<div class="row project">
			<div class="col-md-6">
				<p>My career journey began in Los Angeles at <a href="https://designcodedeploy.com">Plural</a>, right after completing my Bachelor's in Graphic Design, while I was still based in Santiago de Chile. This early experience taught me the ropes of remote work and instilled a strong work ethic.</p>
				<p>Subsequently, I joined <a href="https://www.jelly.cl">Jelly digital</a>, an agency specializing in Public Relations, which greatly enhanced my communication skills. My role involved creating a wealth of content, especially for Google Plus, a platform from days gone by.</p>
				<p>The next chapter in my career led me to <a href="https://www.ilogica.cl">Ilogica</a>, where I was entrusted with building their design team and internal processes. This role was an absolute joy; we crafted numerous design systems and websites for a variety of companies.</p>
				<a href="https://dribbble.com/osvaldouribe" class="cta-main">Documented on Dribbble</a>
			</div>
		</div>
	</section><!--Section-->
    
    <div style="background:#C2D0FC; margin:4px; border-radius:8px;">
	<section class="container">
		<div class="row">
			<div class="col-md-4 col-sm-12">
				<h2>Revitalizing Cities with Lightspeed's Retail POS</h2>
				<p class="timeline">Jul 2018 - Jul 2020</p>
                <p>My journey with <a href="https://www.lightspeedhq.com">Lightspeed</a> provided me with a remarkable opportunity – taking ownership of their Retail Point of Sale for iOS. This experience was invaluable.

				<p>During this time, I had the privilege of collaborating with numerous internal stakeholders and various user types. I played a key role in connecting them and facilitating the most significant product release in many years. This achievement earned me an internal promotion, allowing me to collaborate closely with the Sales team to revamp their processes.</p>
                
				<a href="lightspeed.php" class="cta-main">Read the full story</a>
			</div>
			<div class="col-md-6 offset-md-2">
				<img src="img/project-lightspeed.png" class="rounded img-fluid">
			</div>
		</div>
	</section>
	</div><!--Project-->
	
    <div style="background:#D3D4F5; margin:4px; border-radius:8px;">
	<section class="container">
		<div class="row">
			<div class="col-md-4">
				<h2>Versatile Roles at Opslock</h2>
				<p class="timeline">Jul 2020 - Jul 2021</p>
                <p>During my time at <a href="https://www.opslock.com">Opslock</a>, I took on various roles. I spearheaded the development of a comprehensive Design System, while also nurturing a Design Thinking culture within the organization. This endeavor was particularly rewarding, as it involved building a non-existent product that harnessed artificial intelligence to calculate real-time risk.</p>

				<p>In parallel, I facilitated discovery sessions to chart the current Customer Journey as part of our commitment to assisting customers in their digitalization processes.</p>
				<a href="opslock.php" class="cta-main">Check out the Opslock project</a>
			</div>
			<div class="col-md-6 offset-md-2">
				<img src="img/project-opslock.jpg" class="rounded img-fluid">
			</div>
		</div>
	</section>
	</div><!--Project-->
	
    <div style="background:#F6C4E6; margin:4px; border-radius:8px;">
	<section class="container">
		<div class="row">
			<div class="col-md-4">
				<h2>Mural: Empowering On-the-Go Collaboration</h2>
				<p class="timeline">Jul 2021 - Aug 2022</p>
				<p><a href="https://www.mural.co">Mural</a> is undoubtedly one of the most incredible web apps I've ever encountered. It doesn't just enable you to visualize your ideas, but it also facilitates real-time collaboration with others.</p>
                <p>My role revolves around ensuring that this seamless collaboration extends to mobile and tablet users. I focus on optimizing the interface for these platforms, allowing people to work together wherever they are.</p>
				<a href="mural.php" class="cta-main">Start collaborating</a>
			</div>
			<div class="col-md-6 offset-md-2">
				<img src="img/project-mural.jpg" class="rounded img-fluid">
			</div>
		</div>
	</section>
	</div><!--Project-->
	
	<div style="background:#D9DDC9; margin:4px; border-radius:8px;">
	<section class="container">
		<div class="row">
			<div class="col-md-4">
				<h2>Empowering the Hospitality Industry with a Suite of Products at Life House</h2>
				<p class="timeline">Sep 2022 – Current</p>
				<p>Our journey at <a href="https://www.life-house.com">Life House</a> began by standardizing the User Interface, a crucial first step. We then elevated it through close collaboration with the Engineering team to expedite the development process. This collaborative effort allowed us to swiftly bring our products to market, where we could finally engage in meaningful conversations with our customers.</p>
				<a href="lifehouse.php" class="cta-main">Work In Progress</a>
			</div>
			<div class="col-md-6 offset-md-2">
				<img src="img/project-lifehouse.png" class="rounded img-fluid">
			</div>
		</div>
	</section>
	</div><!--Project-->
	<section class="container">
		<div class="row project">
			<div class="col-md-8">
				<h2>Alongside my professional work, I've also delved into the world of art.</h2>
                <a href="https://www.instagram.com/art.osv/" target="_blank" class="cta-main">Check out my Artwork!</a>
			</div>
		</div><!--Project-->
	</section>
	<?php include 'footer.php'; ?>
  </body>
</html>